...
import type { TeacherListQuery } from '../types';
...
export function useTeacherListParams() {
  ...
  const query: TeacherListQuery = {
    ...
    classId: parseString(searchParams.get('classId')),
    status: (parseString(searchParams.get('status')) as TeacherListQuery['status']) ?? 'all',
  };

  const setQueryParams = (updates: Partial<Record<keyof TeacherListQuery | 'page', string | number | undefined>>) => {
  ...
  const setFilters = (filters: Partial<Omit<TeacherListQuery, 'pageNumber' | 'pageSize'>>) => {
